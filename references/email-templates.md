# Email Templates — 4-Week Sequence

Every email is multipart: plain text body + HTML signature from `assets/signature.html`.

## Shared Rules

- Keep body concise — Email 1 runs longer due to the service list. Emails 2-4: 3-5 sentences, ~75 words max.
- No images, no HTML in body, no tracking pixels.
- Plain text only for the message body. HTML only in signature.
- Contractions, natural language — write like you talk.
- Mandatory humanizer pass on every email before sending.
- `{first_name}` is the only merge field — pulled from database. No per-contact AI research needed.
- Emails are audience-generic but audience-specific — all recipients are GMs at dealerships, so the messaging fits everyone without individual customization.

---

## Email 1 (Week 1) — Boots on the Ground Intro
...
Appended to every email. Source: `assets/signature.html`

```
Gabby Pals
Solutions Consultant
Paramount Lead Solutions
TrafficDriver.ai
800-643-5084
```
